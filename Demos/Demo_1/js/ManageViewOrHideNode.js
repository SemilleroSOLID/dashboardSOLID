...
function isAnyNodeVisible(childNodesChild, nodeRoot) {
  for (let r = 0; r < childNodesChild.length; r++) {
    if (nodes.get(childNodesChild[r]).level == nodes.get(nodeRoot).level + 1) {
      return nodes.get(childNodesChild[r]).hidden;
    }
  }
  return false;
}
...
function nodesNextNodeId(nodeRoot) {
  ...
  for (let i = 0; i < listNodesChilds.length; i++) {
    if (
      ...
      nodes.get(nodeRoot).name == nodes.get(listNodesChilds[i]).name
    ) {
...
function manageNodesHideOrView(network, nodeRoot,nodes,edges) {
  ...
  if (isAnyNodeVisible(nodeChildsAll, nodeRoot) == false) {
    console.log("___________Hide____________"+nodes[0]);
    hideNodes(nodeRoot);
  } else {
    console.log("___________view____________"+edges[0]);
    viewNode(nodeRoot);
  }
  network.fit();
}
...
/*network.on("click", function (e) {
  if (e.nodes.length != 0) {
    manageNodesHideOrView(network, e.nodes[0]);
